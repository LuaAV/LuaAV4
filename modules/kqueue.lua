...
function kqueue.watch(self, filename, cb)
	local fd = C.open(filename, C.O_EVTONLY)
	...
	if C.kevent(self, evSet, 1, nil, 0, nil) == -1 then
		kevent_errno()
	end
...
function kqueue.unwatch(self, w)
...
function kqueue.start(self, cb)
	...
	while true do
		...
		local nev = C.kevent(self, nil, 0, evList, 32, timeout)
		for i = 0, nev-1 do
			local ev = evList[i]
